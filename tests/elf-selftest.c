 ...
 * constructor in there is gated on foreign_dlopen_mode(), which is false
 * unless ANYLINUX_LIB_FOREIGN_DLOPEN=1 is set, and this test never sets it.
 *
 *   cc -O2 -o elf-selftest tests/elf-selftest.c -ldl
 *   ./elf-selftest <some.so>
 */
#include "../src/foreign-dlopen.c"
...
static int count_version_tags(const struct fgn_elf *e) {
	int n = 0;
...
static void test_strip(const char *path) {
	...
	char tmp[] = "/tmp/fgn-selftest-XXXXXX";
	int fd = mkstemp(tmp);
	if (fd >= 0) {
		ssize_t w = write(fd, a.map, a.size);
		close(fd);
		CK("T0.4 stripped image writes out", w == (ssize_t)a.size);
		CK("T0.4 stripped image re-parses", fgn_parse_elf(&b, tmp) != 0);
		if (b.map) {
	...
	unsigned h1 = fgn_content_hash(path, orig_size);
	unsigned h2 = fgn_content_hash(path, orig_size);
...
static void test_tail_merge_guard(const char *path) {
	struct fgn_elf e;
	if (!fgn_parse_elf(&e, path) || !e.dynsym || !e.strtab) {
		printf("  SKIP T0.7 - no usable .dynsym in %s\n", path);
		if (e.map) fgn_free_elf(&e);
		return;
	}
	...
	CK("T0.7 guard REFUSES a range containing another live reference",
	   fgn_dynstr_range_occupied(&e, lo, hi + 1, lo) == 1);
	...
	for (size_t i = 0; i < e.dynsym_num && !found_clear; i++) {
		...
		if (len < 2)
			continue;
		if (!fgn_dynstr_range_occupied(&e, o + 1, o + len + 1, o))
			found_clear = 1;
	}
	...
	CK("T0.7 shipped rename is a suffix identity (no write needed)",
	   strcmp(fgn_renames[0].from + (strlen(fgn_renames[0].from) -
	                                 strlen(fgn_renames[0].to)),
	          fgn_renames[0].to) == 0);
...
static void test_fuzz(const char *path) {
	...
	char tmp[64];
#define FRESH_TMP() strcpy(tmp, "/tmp/fgn-fuzz-XXXXXX")
		...
		if (w == cut) {
			...
			if (fgn_parse_elf(&e, tmp)) {
				...
				if (e.dynsym_num <= e.size / sizeof(ElfW(Sym)) &&
					...
					survived++;
					fgn_apply_renames(&e, 1);   /* dry run: must not crash */
				}
				fgn_free_elf(&e);
			} else {
