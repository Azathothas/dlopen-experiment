#!/usr/bin/env python3
"""
Generate the forward-compatibility shim (PROMPT.md 5.2, Design B).
...
IRRELEVANT_RE = [
    ...
    # ships them in libcrypt.so.1 -- which foreign-dlopen.c now loads into the
 ...
 *
 * Forward-compatibility shim (PROMPT.md 5.2, Design B).
 *
 ...
 * this file was generated -- that is Design R's job (PROMPT.md 4.5, E12).
 *
...
static void shim_fatal(const char *sym, const char *why) {
	...
	static const char p1[] = "\n[foreign-dlopen] FATAL: ";
	static const char p2[] = "\n[foreign-dlopen] the bundled glibc " SHIM_FLOOR_RELEASE;
	static const char p3[] = " does not provide this symbol\n"
	                         "[foreign-dlopen] and no implementation exists for it. "
	                         "Set ANYLINUX_RUNTIME=host to run\n"
	                         "[foreign-dlopen] against the host's own libc, "
	                         "which will have it.\n";
...
def render(floor, target, gap, decided, args):
    ...
    out.append("/* If the bundled glibc was upgraded without regenerating this file we\n"
               ...
               "   say so loudly under ANYLINUX_LIB_DEBUG=1 rather than fail mutely. */")
...
def main():
    ...
    manifest = {
        ...
        "note": ("Covers only the enumerable gap. A symbol introduced after "
                 ...
                 "see PROMPT.md 4.5. Design R (host-runtime switch) is the "
