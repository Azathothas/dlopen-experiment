 ...
 *
 * Environment:
 *   ANYLINUX_RUNTIME=host|bundled|auto   force the decision (default auto)
 *   ANYLINUX_LIB_DEBUG=1                 log the decision and its reason
 ...
 * chain walk. Same problem, and same solution, as in foreign-dlopen.c. */
...
static void rs_decide(struct rs_plan *p, const char *appdir) {
	char path[RS_MAX_PATH];
	const char *forced = getenv("ANYLINUX_RUNTIME");
	...
	if (forced && !strcmp(forced, "host")) {
		...
		rs_log("ANYLINUX_RUNTIME=host: forced, still verifying the set\n");
	} else if (forced && !strcmp(forced, "bundled")) {
		snprintf(p->reason, sizeof(p->reason),
		         "ANYLINUX_RUNTIME=bundled -- forced by the user");
		return;
	} else if (forced && *forced && strcmp(forced, "auto")) {
		snprintf(p->reason, sizeof(p->reason),
		         "ANYLINUX_RUNTIME=%s is not host|bundled|auto; using bundled", forced);
		return;
	}
	...
	snprintf(blib, sizeof(blib), "%s/lib", appdir);
	if (!rs_exists(blib, "libc.so.6", path, sizeof(path))) {
		snprintf(p->reason, sizeof(p->reason),
		         "no bundled libc.so.6 under %s -- not an anylinux AppDir", blib);
		return;
	}
...
static int rs_build_farm(const struct rs_plan *p, char *out, size_t outsz) {
	...
	snprintf(out, outsz, "%s/.anylinux-hostrt-%s", base,
...
int main(int argc, char **argv) {
	rs_debug = getenv("ANYLINUX_LIB_DEBUG") &&
	           !strcmp(getenv("ANYLINUX_LIB_DEBUG"), "1");
