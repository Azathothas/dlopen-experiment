...
static void vc_fatal(const char *sym) {
	...
	fprintf(stderr, " [foreign-dlopen.so] >> version-compat: %s did not resolve\n", sym);
	abort();
}
...
static void *vc_resolve(const char *sym) {
	...
	if (getenv("ANYLINUX_LIB_DEBUG"))
		fprintf(stderr, " [foreign-dlopen.so] >> version-compat: %s@%s -> %p\n",
